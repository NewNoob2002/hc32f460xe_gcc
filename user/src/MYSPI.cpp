...
void Spi_DmaConfig(void)
{
  ...
  stcDmaCfg.u32SrcAddr = static_cast<uint32_t>(0);
  stcDmaCfg.u32DesAddr = reinterpret_cast<uint32_t>(&SPI_UNIT->DR);
...
void spi_dma_push(const void *data, const uint32_t size)
{
  DMA_Cmd(SPI_DMA_UNIT, Disable);
  SPI_Cmd(SPI_UNIT, Disable);
  DMA_SetSrcAddress(SPI_DMA_UNIT, SPI_DMA_TX_CHANNEL, reinterpret_cast<uint32_t>(data));
